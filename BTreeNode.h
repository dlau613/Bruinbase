...
class BTLeafNode {
  public:
    static const int MAX_KEYS = 83;    // the size of a page is 1KB
    BTLeafNode();
    RC printBuf();
   /**
    ...
    */
    RC insert(int key, const RecordId& rid, int split = 0);
...
class BTNonLeafNode {
  public:
    // MAX_KEYS is the number of key/pid pairs that can be inserted
    // However, note that there are 127 pids because the first pid stored is separate
    static const int MAX_KEYS = 126;    // the size of a page is 1KB
    BTNonLeafNode();
    RC printBuf();
    ...
    * @return 0 if successful. Return an error code if the node is full.
    */
    RC insert(int key, PageId pid, int split = 0);
