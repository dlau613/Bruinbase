...
class BTreeIndex {
 public:
  BTreeIndex();
  RC printRoot();
  RC printPid(PageId pid, int leaf);
  RC printIndex();
  RC printIndexRec(PageId pid, int currentHeight);
  RC printBuf();
  int getTreeHeight();
  /**
  ...
  RC insertHelper(PageId pid, int key, const RecordId& rid, int currentHeight,int& siblingKey, PageId& newNodePid);
  /**
  ...
  RC locateHelper(int searchKey, PageId pid, IndexCursor& cursor, int currentHeight);
  /**
 ...
 private:
  ...
  // to store rootPid and treeHeight to disk when closing index and restore it when opening
  // will be stored at pid = 0
